//
//  CSFloatingCollectionScene.swift
//  Bubbles
//
...
@objc public protocol CSFloatingCollectionSceneDelegate {
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, shouldSelectFloatingNodeAtIndex index: Int) -> Bool
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, didSelectFloatingNodeAtIndex index: Int)
    
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, shouldDeselectFloatingNodeAtIndex index: Int) -> Bool
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, didDeselectFloatingNodeAtIndex index: Int)
    
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, startedRemovingOfFloatingNodeAtIndex index: Int)
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, canceledRemovingOfFloatingNodeAtIndex index: Int)
    
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, shouldRemoveFloatingNodeAtIndex index: Int) -> Bool
    @objc optional func floatingScene(_ scene: CSFloatingCollectionScene, didRemoveFloatingNodeAtIndex index: Int)
}

public enum CSFloatingCollectionSceneMode {
    case normal
    case editing
    case moving
}

open class CSFloatingCollectionScene: SKScene {
    fileprivate(set) open var magneticField = SKFieldNode.radialGravityField()
    fileprivate(set) var mode: CSFloatingCollectionSceneMode = .normal {
        didSet {
            modeUpdated()
        }
    }
    fileprivate(set) open var floatingNodes: [CSFloatingNode] = []
    ...
    open weak var floatingDelegate: CSFloatingCollectionSceneDelegate?
    ...
    override open func update(_ currentTime: TimeInterval) {
        ...
        if let tStartTime = touchStartedTime, let tPoint = touchPoint {
            ...
            if dTime >= timeToStartRemove {
                ...
                if let node = atPoint(tPoint) as? CSFloatingNode {
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if mode != .moving, let touch = touchPoint {
            if let node = atPoint(touch) as? CSFloatingNode {
    ...
    fileprivate func cancelRemovingNode(_ node: CSFloatingNode!) {
        mode = .normal
    ...
    open func floatingNodeAtIndex(_ index: Int) -> CSFloatingNode? {
    ...
    override open func atPoint(_ p: CGPoint) -> SKNode {
        ...
        while !(currentNode.parent is SKScene) && !(currentNode is CSFloatingNode)
    ...
    fileprivate func startRemovingNode(_ node: CSFloatingNode!) {
        mode = .editing
    ...
    fileprivate func updateNodeState(_ node: CSFloatingNode!) {
        if let index = floatingNodes.index(of: node) {
            switch node.state {
            case .normal:
    ...
    override open func addChild(_ node: SKNode) {
        if let child = node as? CSFloatingNode {
            configureNode(child)
    ...
    fileprivate func configureNode(_ node: CSFloatingNode!) {
